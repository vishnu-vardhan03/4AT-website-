...
export default function EsslLoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(false);
    if (!result?.ok) {
      setError(accessType === "technician" ? "The IT support email or password is incorrect." : "This email cannot use employee access.");
      return;
    }
    const callbackUrl = new URLSearchParams(window.location.search).get("callbackUrl");
    router.replace(callbackUrl?.startsWith("/essl") ? callbackUrl : "/essl");
    router.refresh();
  }
    ...
    <main className="services-page relative grid min-h-dvh place-items-center overflow-hidden bg-background px-5 py-10 text-foreground">
      ...
      <section className="relative w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-[#05081c] shadow-[0_24px_70px_rgba(0,0,0,.45)]">
        ...
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-2 text-sm leading-6 text-sky-100/65">Employees raise and track requests. IT support manages the technician queue.</p>
          <div className="mt-6 grid grid-cols-2 rounded-xl border border-white/10 bg-[#0a0d24] p-1" aria-label="Choose access type">
            {(["employee", "technician"] as const).map((type) => <button key={type} type="button" onClick={() => { setAccessType(type); setError(""); setEmail(type === "technician" ? "itsupport@consult-4at.com" : ""); setPassword(""); }} aria-pressed={accessType === type} className={`min-h-11 rounded-lg px-3 py-2.5 text-sm font-bold capitalize transition ${accessType === type ? "bg-sky-500 text-[#01030e]" : "text-sky-100/60 hover:text-white"}`}>{type === "technician" ? "IT support" : "Employee"}</button>)}
          </div>
          <form onSubmit={submit} className="mt-7">
            ...
            <p id="email-help" className="mt-2 text-xs text-sky-100/55">{accessType === "employee" ? "Allowed domains: @consult-4at.com and @4at.ai" : "Restricted IT support account"}</p>
