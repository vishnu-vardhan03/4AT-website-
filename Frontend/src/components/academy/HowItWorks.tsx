...
const TRAVEL      = 0.5;  // seconds between nodes
const HOLD_START  = 0.5;  // hold at Stage 01 before particle moves
const HOLD_FWD    = 0.3;  // hold at Stage 06 before reset
const RESET_DUR   = 0.1;  // simultaneous reset fade duration
const N           = 5;    // number of connecting segments

const FWD_END    = HOLD_START + N * TRAVEL;  // 3.0 s — forward pass done
const HOLD_END   = FWD_END + HOLD_FWD;       // 3.3 s — hold ends, reset begins
const RESET_END  = HOLD_END + RESET_DUR;     // 3.4 s — reset done, cycle repeats
...
export function HowItWorks({ sectionId = "selection-metrics" }: { sectionId?: string }) {
  ...
  useLayoutEffect(() => {
    ...
    if (isDesktop) {
      ...
      // ── FORWARD PASS  t = HOLD_START → FWD_END (0.5 → 3.0 s) ───────────────
      // The particle sits at Stage 01 from t=0 → t=HOLD_START (0.5 s),
      // letting the first node be seen before movement begins.
      for (let i = 0; i < N; i++) {
        const t  = HOLD_START + i * TRAVEL;  // offset every segment by the initial hold
        ...
        tl.to(`.node-wrapper-${i}`,        { opacity: 0.9, duration: 0.18 }, t + 0.06);
        tl.to(`.node-circle-${i}`,         { scale: 1.0, background: "rgba(7,9,13,0.45)", borderColor: cc, boxShadow: `0 0 12px ${cc}38`, duration: 0.18 }, t + 0.06);
        tl.to(`.node-icon-${i}`,           { filter: "brightness(0.9)", duration: 0.18 }, t + 0.06);
        tl.to(`.active-ring-overlay-${i}`, { opacity: 0, scale: 0.8, duration: 0.18 }, t + 0.06);
        tl.to(`.active-halo-overlay-${i}`, { opacity: 0, duration: 0.18 }, t + 0.06);
        tl.to(`.stage-text-block-${i}`,    { opacity: 0.7, y: 0, duration: 0.18 }, t + 0.06);
        tl.to(`.stage-label-${i}`,         { color: "rgba(255,255,255,0.5)", duration: 0.18 }, t + 0.06);
        tl.to(`.stage-title-${i}`,         { color: "rgba(255,255,255,0.7)", duration: 0.18 }, t + 0.06);
        tl.to(`.stage-desc-${i}`,          { color: "rgba(255,255,255,0.6)", duration: 0.18 }, t + 0.06);

        // Node ni → active (glow up, text brighten)
        tl.to(`.node-wrapper-${ni}`,        { opacity: 1.0, duration: 0.18 }, t + 0.10);
        ...
        tl.to(`.node-icon-${ni}`,           { color: nc, scale: 1.08, filter: "brightness(1.25)", duration: 0.18 }, t + 0.10);
        tl.to(`.active-ring-overlay-${ni}`, { opacity: 0.18, scale: 1, duration: 0.18 }, t + 0.10);
        tl.to(`.active-halo-overlay-${ni}`, { opacity: 0.15, duration: 0.18 }, t + 0.10);
        tl.to(`.stage-text-block-${ni}`,    { opacity: 1.0, y: 0, duration: 0.18 }, t + 0.10);
        tl.to(`.stage-label-${ni}`,         { color: nc, duration: 0.18 }, t + 0.10);
        tl.to(`.stage-title-${ni}`,         { color: "#ffffff", duration: 0.18 }, t + 0.10);
        tl.to(`.stage-desc-${ni}`,          { color: "rgba(255,255,255,0.85)", duration: 0.18 }, t + 0.10);

        // Impact burst + scale pulse on arrival
        const arrT = t + TRAVEL - 0.12;
        tl.call(() => triggerNodeBurst(ni), [], arrT);
        tl.to(`.node-circle-${ni}`, { scale: 1.08 * 1.15, filter: "brightness(1.3)", duration: 0.07 }, arrT);
        tl.to(`.node-circle-${ni}`, { scale: 1.08, filter: "brightness(1.0)", duration: 0.10, ease: "power3.out" }, arrT + 0.07);
      }

      // Intensify Stage 06 glow while holding at end of cycle
      tl.to(".node-circle-5",         { boxShadow: `0 0 40px ${NODE_COLORS[5]}aa, 0 0 70px ${NODE_COLORS[5]}44`, duration: 0.20 }, FWD_END);
      tl.to(".active-halo-overlay-5", { opacity: 0.35, duration: 0.20 }, FWD_END);

      // ── HOLD at Stage 06 (FWD_END → HOLD_END, 0.3 s) ────────────────────
      tl.to(_hold, { v: 1, duration: HOLD_FWD }, FWD_END);

      // ── SIMULTANEOUS RESET (HOLD_END → RESET_END, 0.1 s) ───────────────
