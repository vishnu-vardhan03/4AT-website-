...
4. Run `npm ci` and `npm run check` in both `Backend` and `Frontend`.
   The frontend uses Next.js standalone output. Its `postbuild` step stages
   `public` and `.next/static` inside `.next/standalone`, and the Docker image
   preserves the same layout. Start a bare EC2 release with `npm start`, which
   runs the generated `.next/standalone/server.js`. Do not use `next start`.
